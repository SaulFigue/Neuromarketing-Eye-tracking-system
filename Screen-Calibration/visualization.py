...
######### AQUI REVISA PARA SACAR LAS COORDENADAS DE LOS OJOS ------------
...
def main(base_path: str, screen_height_mm_offset: int = 10):
    ...
    for idx, row in df.iterrows():
        ...
        monitor_mm = tuple(map(int, row['Monitor_mm'][1:-1].split(',')))
        monitor_pixels = tuple(map(int, row['Monitor_pixels'][1:-1].split(',')))
        point_on_screen_px = tuple(map(int, row['GT_Screen_P_pxl'][1:-1].split(',')))
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f'./data/p00/3D_plots/3d_plot_{idx}.png')
        plt.show()
