...
def add_gazepoint(point_on_screen: np.ndarray,path: str):
    """
    Open the gazepoints.csv file and add 
    row by row the 2D gaze points (x,y)
    """

    with open(path, "a", newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(point_on_screen)
...
def ray_plane_intersection(support_vector: np.ndarray, direction_vector: np.ndarray, plane_normal: np.ndarray, plane_d: np.ndarray) -> np.ndarray:
    ...
    #This is kind of the new Rotation matrix
    ...
    #This is kind the new Translation matrix
