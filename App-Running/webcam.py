...
class WebcamSource:
    ...
    # def __init__(self, camera_id=0, width=1366, height=768, fps=30, buffer_size=1):
    def __init__(self, camera_id=0, width=1280, height=720, fps=30, buffer_size=1):
        ...
        # self.__capture.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'mp4v'))
    ...
    def __next__(self):
        ...
        # ---------------------------
        # if ret == True:
        #     cv2.VideoWriter(f'{datetime.now().strftime("%Y-%m-%d_%H:%M:%S")}.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (1280, 720)).write(frame)
        # ---------------------------
